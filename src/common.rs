...
// Parses a SQL identifier (alphanumeric1 and "_").
fn sql_identifier_with_slash(i: &[u8]) -> IResult<&[u8], &[u8]> {
    alt((
        preceded(
            not(peek(sql_keyword)),
            take_while1(is_sql_identifier_or_slash),
        ),
        delimited(tag("`"), take_while1(is_sql_identifier), tag("`")),
        delimited(tag("["), take_while1(is_sql_identifier), tag("]")),
    ))(i)
...
#[inline]
pub fn is_sql_identifier_or_slash(chr: u8) -> bool {
